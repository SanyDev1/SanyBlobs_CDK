...
namespace MudBun
{
  public class FloaterNew : MonoBehaviour
  {
    ...
    void FixedUpdate()
    {
      ...
      if (UseLocalSpace)
      {
        transform.localPosition = Vector3.MoveTowards(transform.position, m_hoverCenter + hoverVec, (float)0.008);
        transform.localRotation = m_hoverRot * hoverQuat;
      }
      else
      {
        transform.position = Vector3.MoveTowards(transform.position, m_hoverCenter + hoverVec, (float)0.008);
